...
module.exports = (req,res) => {
    ...
    request.request(url,'GET','',header).then((result)=>{
        ...
        filesize = str.subStringMulti(result,'id="file_size">','</span>').toString()
        fileid = str.subStringMulti(result,"dolike&file_id=","'").toString()
        if(fileid == ''){
            res.send({
                code:-1,
                message:"未解析出文件id"
        ...
        request.request('https://www.77file.com/ajax.php','POST','action=load_down_addr1&file_id='+fileid,header).then((result)=>{
